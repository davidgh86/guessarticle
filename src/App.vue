...
export default {
  ...
  data() {
    return {
      apiUrl: process.env.VUE_APP_API_REST_URL,
      numberOfGuessedWords: 0,
      finished: false,
      wordsRegistry: new Map(),
      guessedWords: new Set([]),
      title: [],
      propositions: new Set(["a", "ante", "bajo", "cabe", "con", "contra", "de", "desde", "durante", 
      "en ", "entre", "hacia", "hasta", "mediante", "para", "por", "según", "sin", "so", "sobre", "tras", "versus", "vía", "ahora", "antes", "después", "tarde", "luego", "ayer", "hoy", "mañana", "todavía", "aún", "nunca", "enseguida", "mientras", "después", "temprano",
"aquí", "lejos", "cerca", "fuera", "detrás encima", "ahí", "arriba",
"bien", "mal", "lentamente", "según", "mejor", "peor",
"tanto", "mucho", "demasiado", "más", "poco", "menos", "nada", "todo",
"no", "tampoco", "nada", "nunca", "jamás",
"sí", "también", "asimismo", "bueno", "seguro", "naturalmente", "verdaderamente",
"entonces", "así", "tal", 
"donde", "cuando", "cuanto", "como"]),
  ...
  mounted(){
    fetch(this.apiUrl+"/text")
    .then(response => response.json())
    .then(data => {
      this.title = data.title
      this.words = data.words
      this.separators = new Set(data.separators)
